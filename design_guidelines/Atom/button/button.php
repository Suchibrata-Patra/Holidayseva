...
<span class="t">&lt;/div&gt;</span></code></pre>
    ...
    <!-- ════════════════════════════════════════
         COLORS.CSS ADDITIONS REQUIRED
    ════════════════════════════════════════ -->
    <section id="tokens">
      <h2 class="section-title">Required <code>colors.css</code> Additions</h2>
      <p class="body-text">
        The button system references the existing tokens in <code>colors.css</code> and
        declares its own in <code>button.css :root</code>. However, the tokens below
        should also be added to your <strong>central</strong> <code>colors.css</code> so
        other components can reference them consistently:
      </p>

      <div class="colors-additions">
        <h3>📋 Add to colors.css :root { … }</h3>
        <div class="code-block">
          <div class="code-bar"><span class="code-lang-tag">CSS</span><button class="copy-btn" onclick="copyCode(this)">Copy</button></div>
...
│           └── button.js         ← Optional interactions
│
└── design_guidelines/
    └── Atom/
        └── button/
            └── button.php        ← This documentation page</code></pre>
      </div>

      <div class="note info">
        <p>
          Import <code>button.css</code> after <code>colors.css</code> so that all
          <code>--color-*</code> tokens are defined before the button system references them.
        </p>
      </div>
    </section>
